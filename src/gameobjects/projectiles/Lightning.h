...
struct Lightning final : Projectile {
  ...
  Lightning(const Point& pos, bool from_player, int life_span, float speed, float damage,
           HitType hit_type, const std::array<StatusEffect*,MAX_STATUS_EFFECTS_PRJ>& effects, float pov,
  ...
  void draw() final {
    DrawTextureProFastRotOffset(resources->frames[spriteCounter % 60 / 15], pos.x_ + DRAW_X,
