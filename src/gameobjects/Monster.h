...
struct Monster : public Entity {
  ...
  void draw() override {
    if (shape_type == ShapeType::CIRCLE) {
      DrawCircleSector(pos.x() + DRAW_X, pos.y() + DRAW_Y, size.x(), 0, 360, 50, PURPLE);
    } else {
      DrawRectanglePro(pos.x() + DRAW_X, pos.y() + DRAW_Y, size.x(), size.y(), {0, 0},
                       pov, PURPLE);
    }
  ...
  bool move_to_player() noexcept {
    ...
    if ((point = astar_pathfinding(tile_pos, *PLAYER_TILE)) > 0) [[likely]] {
      ...
      if (pos.x() < PLAYER_X) {
        pos.x() += speed;
      } else if (pos.x() > PLAYER_X) {
        pos.x() -= speed;
      } else if (pos.y() > PLAYER_Y) {
        pos.y() -= speed;
      } else if (pos.y() < PLAYER_Y) {
        pos.y() += speed;
      }
