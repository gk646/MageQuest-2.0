...
struct MainMenu {
  ...
  MainMenu(SettingsMenu& settings_menu) noexcept
  ...
  void draw() noexcept {
    ...
    const float baseYOffset =
        SCREEN_HEIGHT / 2.0F - 3 * scaled_height - 4 * vertical_gap;
    ...
    if (menu_state == MenuState::Main) {
      if (GuiButton({xOffset, baseYOffset, scaled_width, scaled_height},
                    "Play Game")) {
        GAME_STATE = GameState::Game;
        StopSound(sound::intro);
      }
      if (GuiButton({xOffset, baseYOffset + (scaled_height + vertical_gap),
