...
struct BlastHammer_Skill final : public Skill {
  ...
  void Activate() final {
    ...
    TriggerSkill();
    Point pos = {PLAYER_X + MOUSE_POS.x - CAMERA_X - BlastHammer::width / 2,
                 PLAYER_Y + MOUSE_POS.y - CAMERA_Y - BlastHammer::height / 2};
...
struct FireStrike_Skill final : public Skill {
  ...
  void Activate() final {
    ...
    float damage = PLAYER_STATS.get_ability_dmg(damage_stats) / 60.0F;
